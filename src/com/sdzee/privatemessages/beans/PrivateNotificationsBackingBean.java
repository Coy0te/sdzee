...
/**
 * PrivateNotificationsBackingBean est le bean sur lequel s'appuie le header d'un member connecté pour lister les notifications. Il s'agit
...
public class PrivateNotificationsBackingBean implements Serializable {
    ...
    private List<PrivateNotification> paginatedPrivateNotifications;
    private int                       pagesNumber;
    private int                       page                      = 1;
    private int                       countPrivateNotifications = 0;
    @EJB
     ...
     * déjà initialisé et il est donc possible de faire appel à des dépendances. Ici, c'est le DAO {@link PrivateNotificationDao} injecté
     * via l'annotation <code>@EJB</code> qui entre en jeu.
     * <p>
    ...
    public void init() {
        Member member = (Member) FacesContext.getCurrentInstance().getExternalContext().getSessionMap()
                .get( SESSION_MEMBER );
        if ( member != null ) {
            countPrivateNotifications = privateNotificationDao.count( member );
            pagesNumber = (int) Math.ceil( countPrivateNotifications / NB_NOTIFICATIONS_PER_PAGE );
            paginatedPrivateNotifications = privateNotificationDao.list( member, page, (int) NB_NOTIFICATIONS_PER_PAGE );
        } else {
            paginatedPrivateNotifications = new ArrayList<PrivateNotification>();
        }
    }
     ...
     */
    public List<PrivateNotification> getPaginatedPrivateNotifications() {
        return paginatedPrivateNotifications;
    }
    ...
    public int getCountPrivateNotifications() {
        return countPrivateNotifications;
    }

    public void setCountPrivateNotifications( int countPrivateNotifications ) {
        this.countPrivateNotifications = countPrivateNotifications;
    }
}